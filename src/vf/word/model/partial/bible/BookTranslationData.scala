...
object BookTranslationData extends FromModelFactory[BookTranslationData]
{
	...
	lazy val schema = ModelDeclaration(Pair(
		PropertyDeclaration("book", IntType),
		PropertyDeclaration("translationId", IntType, Single("translation_id"))))
	...
	extends BookTranslationFactory[BookTranslationData] with ModelConvertible
{
	...
	override def toModel = Model(Pair("book" -> book.id, "translationId" -> translationId))

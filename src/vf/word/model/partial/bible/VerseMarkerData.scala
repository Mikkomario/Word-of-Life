...
object VerseMarkerData extends FromModelFactoryWithSchema[VerseMarkerData]
{
	...
	override lazy val schema = ModelDeclaration(Vector(
		PropertyDeclaration("chapterIndex", IntType, Single("chapter_index")),
		PropertyDeclaration("verseIndex", IntType, Single("verse_index")),
		PropertyDeclaration("firstStatementId", IntType, Single("first_statement_id"))))

...
object FootnoteData extends FromModelFactoryWithSchema[FootnoteData]
{
	...
	override lazy val schema = ModelDeclaration(Pair(
		PropertyDeclaration("commentedStatementId", IntType, Single("commented_statement_id")),
		PropertyDeclaration("targetedWordIndex", IntType, Single("targeted_word_index"), isOptional = true)))
...
/**
  * Represents a foot note made within the original text, concerning a specific word or a 
	...
	extends FootnoteFactory[FootnoteData] with ModelConvertible
{
	...
	override def toModel = Model(Pair(
		"commentedStatementId" -> commentedStatementId, "targetedWordIndex" -> targetedWordIndex))
